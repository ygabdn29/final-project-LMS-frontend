...
let AccessAssignment = () => {
  const [dataAssignment, setDataAssignment] = useState(null);
  
  let submissionId = 1; // The submission ID

  useEffect(() => {
    axios.get(`http://localhost:8080/api/course/1/material/1/assignment/1283123/${submissionId}`)
      .then(response => {
  ...
  return (
    <>
      ...
      <h1>Nama Mentee: </h1>
      <h2>{dataAssignment.user.username}</h2>
